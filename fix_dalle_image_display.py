...
# Specific conversation ID with known issues
...
def process_dalle_assets():
    """Process all DALL-E asset references and create placeholder media records"""
    fixed_count = 0
    
    with get_session() as session:
        # Get messages for the specific conversation with the issue
        messages = session.query(Message).filter_by(
            conversation_id=conversation_id
        ).all()
        
        print(f"Processing {len(messages)} messages in conversation {conversation_id}")
